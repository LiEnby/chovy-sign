...
namespace PspCrypto
{
    class RijndaelModTransform : ICryptoTransform
    {
        private bool _isEncrypt;


        private int _blockSizeBits;
        ...
        private int[] _encryptindex = null;
        private int[] _decryptindex = null;

        private int[] _lastBlockBuffer;
        private byte[] _depadBuffer;
        private byte[] _shiftRegister;

        internal RijndaelModTransform(byte[] rgbKey,
            int blockSize, bool isEncrypt)
        {
            if (rgbKey == null)
        ...
        private void Dispose(bool disposing)
        {
            if (disposing)
            {
                ...
                if (_shiftRegister != null)
                {
                    Array.Clear(_shiftRegister, 0, _shiftRegister.Length);
                    _shiftRegister = null;
                }
            }
        }
                    ...
                    fixed (int* T = s_T)
                    {
                        fixed (int* TF = s_TF)
                        {
                            ...
                            for (int blockNum = 0; blockNum < iNumBlocks; ++blockNum)
                            {
                                ...
                                {
                                    Buffer.MemoryCopy(inputPtr + workBaseIndex, work, _blockSizeBytes, _blockSizeBytes);
                                }
        ...
        private unsafe int DecryptData(IReadOnlyList<byte> inputBuffer,
            int inputOffset,
            int inputCount,
            ref byte[] outputBuffer,
            int outputOffset,
            bool fLast)
        {
            if (inputBuffer.Count < inputOffset + inputCount)
                throw new CryptographicException("Cryptography_InsufficientBuffer");
        ...
        {
            switch (_blockSizeBits > rgbKey.Length * 8 ? _blockSizeBits : rgbKey.Length * 8)
            {
                case 128:
                    _Nr = 10;
                    break;
                default:
                    throw new CryptographicException("InvalidKeySize");
            }
